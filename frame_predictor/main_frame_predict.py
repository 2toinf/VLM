...
def main(args):
    ...
    text_encoder = create_model(args.text_encoder)
    ...
    if utils.get_rank() == 0:
        model.eval()
        input_shape = int(model.input_length ** 0.5)
        if args.loss_type == 'kl':
            i = torch.rand(1, model.input_length).long().to(device)
        else:
            i = torch.rand(1, model.in_chans, input_shape,input_shape).to(device)
        flops = FlopCountAnalysis(model, (i, torch.rand(1, 77, 768).to(device)))
        if args.rank == 0:
