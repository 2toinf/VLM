...
from model.Quantizer import NormEMAVectorQuantizer
...
class VQKD(nn.Module):
    def __init__(self,
        ...
        self.encoder_stride =  encoder_config['patch_size']
        ...
        if self.teacher_model_type == 'clip':
            ...
            self.decoder_out_dim = 3 * encoder_config['patch_size']**2
    ...
    def calculate_rec_loss(self, rec, target):  
        if self.rec_loss_type == 'cosine':
            ...
            rec_loss = torch.nn.functional.mse_loss(rec, target)
        else:
...
@register_model
def vqkd_encoder_base_decoder_3x768x12_pixel(pretrained=False, pretrained_weight=None, as_tokenzer=False, img_size=224, 
                                            n_code=8192, code_dim=32, **kwargs):
    encoder_config, decoder_config = get_model_default_params(), get_model_default_params()
...
@register_model
def vqkd_encoder_base_decoder_9x768x12_pixel(pretrained=False, pretrained_weight=None, as_tokenzer=False, img_size=224, 
                                            n_code=16384, code_dim=32, **kwargs):
    encoder_config, decoder_config = get_model_default_params(), get_model_default_params()
