        ...
        if args.resume:
            ...
            for key, value in checkpoint['model'].items():
                if key in model_state_dict:
                    if not model_state_dict[key].shape == value.shape:
                        print(f"Error when loading parameters for {key}, SKIP!")
                    else:
                        new_state_dict[key] = value
            print(model_without_ddp.load_state_dict(new_state_dict, strict=False)) # strict: bool=True, , strict=False
            print("Resume checkpoint %s" % args.resume)
            if args.no_resume_optim:
                print("No resume optim state dict")
                return
